          ...
          "when applied with a Request containing a valid XML body that passes IE507 request schema validation" - {
            ...
            "and the submission is successfully submitted to EIS" in new Setup {
              val request: FakeRequest[NodeSeq] =
                FakeRequest()
                  .withHeaders(CustomHeaderNames.X_CORRELATION_ID -> TestData.correlationId.value)
              ...
              when(
                eisService.submitMessage(
                  ...
                  CorrelationId(eqTo(TestData.correlationId.value))
                )(using any())
              ...
              "with a 500 errorCode" in new Setup {
                ...
                val eisErrorResponseXml: Elem =
                  <errorDetail xmlns="http://www.hmrc.gsi.gov.uk/eis">
                    ...
                    <correlationId>{TestData.correlationId.value}</correlationId>
